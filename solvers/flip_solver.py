...
class flip_solver:
    ...
    def __init__(self,
        ...
        self.block_particle.place(self.x, self.pv)
    ...
    def p2g(self, dt: ti.f32):
        ...
        for p in self.x:
            xp = self.x[p]
            self.p2g1d(stagger_u, xp, self.pv[p].x, self.u, self.u_weight)
            self.p2g1d(stagger_v, xp, self.pv[p].y, self.v, self.v_weight)
            self.p2g1d(stagger_w, xp, self.pv[p].z, self.w, self.w_weight)
    ...
    @ti.func
    def p2g1d(self, stagger, xp, vp, grid_v, grid_m):
        inv_dx = 1 / self.dx
    ...
    def g2p(self, dt: ti.f32):
        ...
        for p in self.x:
            ...
            self.pv[p].x = self.g2p1d(stagger_u, xp, vp.x, self.u, self.u_last)
            self.pv[p].y = self.g2p1d(stagger_v, xp, vp.y, self.v, self.v_last)
            self.pv[p].z = self.g2p1d(stagger_w, xp, vp.z, self.w, self.w_last)

    @ti.func
    def g2p1d(self, stagger, xp, vp, grid_v, grid_v_last):
        inv_dx = 1 / self.dx
        ...
        for i, j, k in ti.static(ti.ndrange(3, 3, 3)):
            ...
            new_v_delta += weight * (grid_v[base + offset] - grid_v_last[base + offset])

        return self.flip_blending * (vp + new_v_delta) + (1 - self.flip_blending) * new_v
